...
// The gap Bridge closes, established by asking both agents what they load from
// a directory holding one tree at a time: opencode reads .claude/skills as well
// as its own, and Claude Code reads only .claude/skills. So a project skill kept
// under .opencode/skills is one the list shows, that opentree links into every
// worktree, and that Claude Code still cannot use.
func TestBridge_MakesARepoSkillReadableByEveryAgent(t *testing.T) {
	...
	if len(made) != 1 {
		t.Fatalf("made = %v, want the other agent's tree", made)
	}

	// One skill still, and now with two readers: the bridge is a second path to
	// the same directory, which Scan collapses.
	after := Scan(repo)
	if len(after) != 1 {
		t.Fatalf("after bridging: %d skills, want the same single skill", len(after))
	}
	if len(after[0].Agents) != 2 {
		t.Errorf("Agents = %v, want both agents to read it", after[0].Agents)
	}
}

// The other direction needs no bridge, and asking for one would put a directory
// in the user's repository that changes nothing. opencode reads Claude Code's
// repository tree directly — which its own documentation does not say, and
// which the first cut of this feature got wrong in the visible direction: it
// warned that a skill under .claude/skills was invisible to opencode while
// opencode was busy answering to it.
func TestBridge_ClaudesTreeIsAlreadyRead(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("XDG_CONFIG_HOME", "")
	repo := t.TempDir()
	...
	if len(found) != 1 || len(found[0].Agents) != 2 {
		t.Fatalf("Scan() = %d skills with agents %v, want one readable by both", len(found), agentsOf(found))
	}
	...
	if len(made) != 0 {
		t.Errorf("made = %v, want nothing — both agents already read that tree", made)
	}
}
